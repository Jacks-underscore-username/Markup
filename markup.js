...
const expandHex = hex => {
  ...
  const prefix = hasPound ? '#' : ''
  if (hex.length === 3) return `${prefix}${hex[0].repeat(2)}${hex[1].repeat(2)}${hex[2].repeat(2)}`
  if (hex.length === 4) return `${prefix}${hex[0].repeat(2)}${hex[1].repeat(2)}${hex[2].repeat(2)}${hex[3].repeat(2)}`
  if (hex.length === 6 || hex.length === 8) return `${prefix}${hex}`
  throw new Error(`expandHex: unexpected hex length ${hex.length} for "${prefix}${hex}"`)
}
